...
// 用正则匹配页面URL的哈希部分(#开头的部分)
// 正则判断是否是16进制数 (i:不区分大小写)
m = location.hash.match(/^#([0-9a-f]{6})$/i);

if (m) 
{
    // 十六进制数字转换为一个介于 0 和 1 的浮点数 (eps 变量用于确定结果的范围和精度)

    // eps 很小的数, 用于缩放结果的范围和精度。值介于 0 和 1 之间
    // m[1].substr(0, 2) 表示哈希部分的前两个字符,
    // 使用 parseInt 函数将其解析为一个十六进制数字

    // 最低偏移量使最小值不为0  [0.001, 254.9491]
...
// 创建Worker对象，加载指定js，此文件将在单独线程运行
let worker = new Worker('worker.js');
// 主线程用 onmessage 监听 Worker线程的 postMessage()消息
